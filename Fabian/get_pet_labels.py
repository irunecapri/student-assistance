...
#filename_list = listdir("pet_images/")  SHOULD NOT HARDCODED BUT ALSO IS BUILT OUTSIDE FUNCTION TO MAY
...
def get_pet_labels(image_dir):
    ...
    results_dic = dict()
    # items_in_dic = len(results_dic) UNUSED VARIABLE
    for idx in range(0, len(in_files), 1):
        if in_files[idx][0] != ".":
            ...
            for word in pet_labels:           
                if word.isalpha():
                    pet_name += word + " "
            pet_labels = pet_name.strip(" ") # INDENTATION MUST BE CHANGED HERE
            """print(pet_labels)"""
